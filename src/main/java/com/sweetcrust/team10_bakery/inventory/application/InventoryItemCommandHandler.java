...
public class InventoryItemCommandHandler {
  ...
  public InventoryItemCommandHandler(
      InventoryItemRepository inventoryItemRepository, ShopRepository shopRepository) {
  ...
  public InventoryItem createInventoryItem(ShopId shopId, AddStockCommand addStockCommand) {
    ...
    boolean foundInventoryItem =
        inventoryItemRepository.existsByShopIdAndVariantId(
            shop.getShopId(), addStockCommand.variantId());

    if (foundInventoryItem) {
      throw new InventoryItemServiceException(
          "InventoryItem", "This inventoryItem already exists in this shop");
    }
