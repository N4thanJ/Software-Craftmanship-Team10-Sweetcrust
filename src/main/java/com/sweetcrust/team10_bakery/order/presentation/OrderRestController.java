...
@Tag(
    ...
    description = "Endpoints related to order status fetching en orderItems")
public class OrderRestController {
  ...
  public OrderRestController(
      OrderCommandHandler orderCommandHandler, OrderQueryHandler orderQueryHandler) {
  ...
  public ResponseEntity<Iterable<Order>> getAllOrdersForCustomer(
      @PathVariable OrderType orderType, @PathVariable UUID userId) {
  ...
  @PostMapping("/B2C")
  public ResponseEntity<Order> createB2COrder(
      @RequestBody CreateB2COrderCommand createB2COrderCommand) {
    Order order = orderCommandHandler.createB2COrder(createB2COrderCommand);
    return ResponseEntity.ok(order);
  }

  @PostMapping("/B2B")
  public ResponseEntity<Order> createB2BOrder(
      @RequestBody CreateB2BOrderCommand createB2BOrderCommand) {
