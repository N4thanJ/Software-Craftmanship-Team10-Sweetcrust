...
public class OrderCommandHandler {
  ...
  public Order createB2COrder(CreateB2COrderCommand createB2COrderCommand) {
    ...
    User user =
        userRepository
            .findById(createB2COrderCommand.customerId())
            .orElseThrow(() -> new OrderServiceException("customerId", "User not found"));

    Cart cart =
        cartRepository
            .findById(createB2COrderCommand.cartId())
    ...
    if (user.getRole() != UserRole.CUSTOMER) {
      throw new OrderServiceException(
          "customerId", "Only users with customer role can make B2C orders");
    }
    ...
    orderEventPublisher.publish(
        new OrderCreatedEvent(savedOrder, user.getEmail(), shop.getEmail()));
  ...
  public Order createB2BOrder(CreateB2BOrderCommand createB2BOrderCommand) {
    ...
    Order savedOrder = orderRepository.save(order);
    orderEventPublisher.publish(
        new OrderCreatedEvent(savedOrder, orderingShop.getEmail(), sourceShop.getEmail()));
    return savedOrder;
  }
  ...
  public Order confirmOrder(OrderId orderId, ShopId sourceShopId, UserId userId) {
    ...
    if (!order.getSourceShopId().equals(sourceShop.getShopId())) {
      throw new OrderServiceException(
          "sourceShopId", "This shop is not the source shop for the order");
    }
    ...
    orderEventPublisher.publish(
        new OrderConfirmedEvent(
            savedOrder, customer != null ? customer.getEmail() : null, sourceShop.getEmail()));
  ...
  public void updateStatusesDaily() {
    ...
    var toShip =
        orderRepository.findByStatusAndRequestedDeliveryDateBetween(
  ...
  public Order cancelOrder(CancelOrderCommand cancelOrderCommand) {
    Order order =
        orderRepository
            .findById(cancelOrderCommand.orderId())
            .orElseThrow(() -> new OrderServiceException("orderId", "Order not found"));

    User user =
        userRepository
            .findById(cancelOrderCommand.userId())
            .orElseThrow(() -> new OrderServiceException("userId", "User not found"));

    if (order.getOrderingShopId() != null) {
      if (user.getRole() != UserRole.BAKER && user.getRole() != UserRole.ADMIN) {
        throw new OrderServiceException(
            "userId", "Only users with baker or admin role can cancel B2B orders");
      }

      Shop orderingShop =
          shopRepository
              .findById(order.getOrderingShopId())
              .orElseThrow(() -> new OrderServiceException("orderingShopId", "Shop not found"));
      Shop sourceShop =
          shopRepository
              .findById(order.getSourceShopId())
              .orElseThrow(() -> new OrderServiceException("sourceShopId", "Shop not found"));
      ...
      Order savedOrder = orderRepository.save(order);
      orderEventPublisher.publish(
          new OrderCancelledEvent(savedOrder, orderingShop.getEmail(), sourceShop.getEmail()));

      return savedOrder;
    } else {
      if (!order.getCustomerId().equals(cancelOrderCommand.userId())
          && user.getRole() != UserRole.ADMIN) {
        throw new OrderServiceException(
            "userId", "Only the customer who placed the order can cancel it");
      }
      ...
      orderEventPublisher.publish(new OrderCancelledEvent(savedOrder, user.getEmail(), null));
