...
public class ProductCommandHandler {
  ...
  public void createProduct(AddProductCommand addProductCommand) {
    ...
    User user =
        userRepository
            .findById(addProductCommand.userId())
    ...
    if (user.getRole() != UserRole.BAKER && user.getRole() != UserRole.ADMIN) {
      throw new ProductServiceException(
          "role", "Only users with BAKER or ADMIN role can create products");
    }
  ...
  public void addVariantToProduct(
      ProductId productId, AddProductVariantCommand addProductVariantCommand) {
    User user =
        userRepository
            .findById(addProductVariantCommand.userId())
            .orElseThrow(() -> new UserServiceException("userId", "User not found"));

    if (user.getRole() != UserRole.BAKER && user.getRole() != UserRole.ADMIN) {
      throw new ProductServiceException(
          "role", "Only users with BAKER or ADMIN role can add product variants");
    }
    ...
    boolean exists =
        product.getVariants().stream()
            .anyMatch(
                variant ->
                    variant.getSize() == addProductVariantCommand.size()
                        && variant.getVariantName().equals(addProductVariantCommand.variantName()));
    if (exists) {
      throw new ProductServiceException(
          "variant", "variant with same size and name already exists");
    }
  ...
  public void markProductUnavailable(MarkProductUnavailableCommand markProductUnavailableCommand) {
    Product product =
        productRepository
            .findById(markProductUnavailableCommand.productId())
            .orElseThrow(() -> new ProductServiceException("product", "product not found"));

    User user =
        userRepository
            .findById(markProductUnavailableCommand.userId())
            .orElseThrow(() -> new ProductServiceException("userId", "User not found"));

    if (user.getRole() != UserRole.BAKER && user.getRole() != UserRole.ADMIN) {
      throw new ProductServiceException(
          "role", "Only users with BAKER or ADMIN role can change product availability");
    }
  ...
  public void markProductAvailable(MarkProductAvailableCommand markProductAvailableCommand) {
    Product product =
        productRepository
            .findById(markProductAvailableCommand.productId())
            .orElseThrow(() -> new ProductServiceException("product", "product not found"));

    User user =
        userRepository
            .findById(markProductAvailableCommand.userId())
            .orElseThrow(() -> new ProductServiceException("userId", "User not found"));
