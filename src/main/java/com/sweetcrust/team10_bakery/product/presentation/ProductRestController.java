...
public class ProductRestController {
    ...
    public ResponseEntity<Void> addVariantToProduct(@PathVariable UUID productId, @RequestBody AddProductVariantCommand addProductVariantCommand) {
        productCommandHandler.addVariantToProduct(new ProductId(productId), addProductVariantCommand);
        return ResponseEntity.noContent().build();
    }

    @PatchMapping("/available")
    public ResponseEntity<Void> markAvailable(@RequestBody MarkProductAvailableCommand markProductAvailableCommand) {
        productCommandHandler.markProductAvailable(markProductAvailableCommand);
        return ResponseEntity.noContent().build();
    }

    @PatchMapping("/unavailable")
    public ResponseEntity<Void> markUnavailable(@RequestBody MarkProductUnavailableCommand markProductUnavailableCommand) {
