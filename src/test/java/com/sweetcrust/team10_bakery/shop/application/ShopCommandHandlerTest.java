...
public class ShopCommandHandlerTest {
  ...
  @InjectMocks private ShopCommandHandler shopCommandHandler;
  ...
  void givenNonexistentUser_whenCreateShop_thenOrderServiceExceptionIsThrown() {
    ...
    AddShopCommand addShopCommand =
        new AddShopCommand(
            "Crust Almighty", address, "contact@crustalmighty.com", new CountryCode("GB"), userId);

    // when
    ShopServiceException exception =
        assertThrows(
            ShopServiceException.class, () -> shopCommandHandler.createShop(addShopCommand));
  ...
  void givenExistingShopName_whenCreateShop_thenShopServiceExceptionIsThrown() {
    ...
    AddShopCommand addShopCommand =
        new AddShopCommand(
            "Dough or Die", address, "info@doughordie.com", new CountryCode("US"), userId);
  ...
  void givenNonAdminUser_whenCreateShop_thenShopServiceExceptionIsThrown() {
    ...
    UserId userId = new UserId();
    User regularUser =
        new User("bread-enthusiast", "ILoveBread123!", "breadlover@bakery.com", UserRole.BAKER);
    Address address =
        Address.builder()
            .setStreet("123 Sourdough Street")
            .setCity("Bread City")
    ...
    AddShopCommand addShopCommand =
        new AddShopCommand(
            "Bake Street Boys", address, "hello@bakeboys.com", new CountryCode("US"), userId);
